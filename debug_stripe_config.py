...
try:
    ...
    if stripe_secret:
        print(
            f"  ✅ STRIPE_SECRET_KEY: {stripe_secret[:7]}...{stripe_secret[-4:] if len(stripe_secret) > 11 else 'TOO_SHORT'}"
        )
    else:
    ...
    if stripe_pub:
        print(
            f"  ✅ STRIPE_PUBLISHABLE_KEY: {stripe_pub[:7]}...{stripe_pub[-4:] if len(stripe_pub) > 11 else 'TOO_SHORT'}"
        )
    else:
    ...
    using_test = any(
        price in test_values for price in [basic_price, premium_price, enterprise_price]
    )
