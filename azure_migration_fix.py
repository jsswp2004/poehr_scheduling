...
def check_migration_status():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if not migrations:
                logger.warning(
                    "⚠️  No communicator migrations found in django_migrations table"
                )
                return False
            ...
            for migration in migrations:
                logger.info(
                    f"   {migration[1]}.{migration[2]} (applied: {migration[3]})"
                )
...
def run_migrations():
    ...
    try:
        ...
        execute_from_command_line(
            ["manage.py", "migrate", "communicator", "--verbosity=2"]
        )
...
def main():
    ...
    logger.info(
        f"   communicator_contact: {'✅ EXISTS' if contact_exists else '❌ MISSING'}"
    )
    logger.info(
        f"   communicator_messagelog: {'✅ EXISTS' if messagelog_exists else '❌ MISSING'}"
    )
    ...
    if run_migrations():
        ...
        if contact_exists and messagelog_exists:
            ...
            logger.warning(
                "⚠️  Migration completed but tables still missing. Trying manual creation..."
            )
