...
try:
    ...
    logger.info(
        f"📋 Number of WebSocket patterns: {len(users.routing.websocket_urlpatterns)}"
    )
    ...
    try:
        ...
        logger.info(
            f"📋 Channel layer backend: {channel_layer.__class__.__module__}.{channel_layer.__class__.__name__}"
        )
