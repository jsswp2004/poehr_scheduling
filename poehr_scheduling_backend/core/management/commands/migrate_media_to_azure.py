...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--dry-run', action='store_true', help='Only print what would happen, no uploads.'
        )
        parser.add_argument(
            '--rewrite-db', action='store_true', help="Normalize DB ImageField paths by stripping '/media/' prefixes."
        )

    def handle(self, *args, **options):
        if BlobServiceClient is None:
            raise CommandError('azure-storage-blob is not installed. Add it to requirements and install.')
        ...
        if conn_str:
            ...
            if not (account_name and account_key):
                raise CommandError('Provide either --connection-string or both --account-name and --account-key.')
        ...
        for file_path in media_root.rglob('*'):
            ...
            try:
                ...
                remote_md5 = None
                if props.content_settings and getattr(props.content_settings, 'content_md5', None):
                    remote_md5 = props.content_settings.content_md5.hex() if isinstance(props.content_settings.content_md5, (bytes, bytearray)) else None
    ...
    def _rewrite_db_paths(self):
        ...
        with transaction.atomic():
            ...
            for u in CustomUser.objects.exclude(profile_picture='').exclude(profile_picture__isnull=True):
