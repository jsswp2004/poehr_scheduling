...
def test_availability_upload():
    ...
    try:
        ...
        if upload_response.status_code == 200:
            response_data = upload_response.json()
            print(f"✅ Created: {response_data.get('created_count', 0)} availability slots")
            print(f"🔄 Updated: {response_data.get('updated_count', 0)} availability slots")
            if response_data.get('errors'):
                print(f"❌ Errors: {response_data['errors']}")
            return True
        else:
...
if __name__ == "__main__":
    ...
    print(f"Availability Upload: {'✅ PASSED' if availability_success else '❌ FAILED'}")
