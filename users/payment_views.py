...
def cancel_subscription(request):
    ...
    try:
        ...
        scheduled_date = data.get('endDate') if cancellation_type == 'scheduled' else None
        ...
        try:
            if user.stripe_subscription_id:
                ...
                logger.info(f"✅ Stripe subscription cancelled: {user.stripe_subscription_id}")
            else:
...
POWER Healthcare IT Team
            ...
            return JsonResponse({
                'success': False,
                'message': 'Failed to cancel account'
            }, status=400)
        ...
        return JsonResponse({
            'success': False,
            'message': f'Account cancellation failed: {str(e)}'
        }, status=500)
...
def change_plan(request):
    ...
    try:
        ...
        return JsonResponse({
            'success': False,
            'message': f'Failed to change plan: {str(e)}'
        }, status=500)
...
def delete_payment_method(request, method_id):
    ...
    try:
        ...
        logger.info(f"🗑️ Payment method deletion requested: {method_id} for user {user.username}")
