    ...
    def _generate_report_data(self, report_type, appointments, availabilities, user, start_dt=None, end_dt=None):
        ...
        if report_type == "Upcoming Appointments Report":
            ...
            upcoming = appointments.filter(
                appointment_datetime__gt=now, status__in=["scheduled", "pending"]
            ...
            return [
                {
                    ...
                    "provider_name": (
                        f"Dr. {apt.provider.first_name} {apt.provider.last_name}"
                        if apt.provider
                        else "TBD"
                    ),
                    "title": apt.title,
                ...
                for apt in upcoming[:100]
            ]  # Limit to 100 records

        elif report_type == "Past Appointments Report":
            now = timezone.now()
            past = appointments.filter(appointment_datetime__lt=now).order_by(
            ...
            return [
                {
                    ...
                    "duration_hours": round(
                        (avail.end_time - avail.start_time).total_seconds() / 3600, 2
                    ),
                }
            ...
            recurring = appointments.exclude(recurrence="none").order_by(
                "appointment_datetime"
            )
            ...
            return [
                {
                    ...
                    "end_date": (
                        apt.recurrence_end_date.strftime("%Y-%m-%d")
            ...
            duration_groups = {
                ...
                "31-60 min": appointments.filter(
                    duration_minutes__gt=30, duration_minutes__lte=60
                ).count(),
                "61-90 min": appointments.filter(
                    duration_minutes__gt=60, duration_minutes__lte=90
                ).count(),
            ...
            return {
                "statistics": {
                    ...
                    "total_duration_hours": round(
                        (duration_stats["total_duration"] or 0) / 60, 1
                    ),
            ...
            daily_counts = (
                appointments.extra(select={"day": "date(appointment_datetime)"})
                .values("day")
            ...
            while current_date <= end_date:
                ...
                trend_data.append(
                    {"date": date_str, "count": daily_dict.get(current_date, 0)}
                )
            ...
            return {
                ...
                "average_daily": round(
                    appointments.count()
                    / max(1, (end_date - start_dt.date()).days + 1),
                    1,
                ),
            }
            ...
            return {
                ...
                "rates": {
                    "completion_rate": round(
                        (completed_count / max(1, total_appointments)) * 100, 1
                    ),
                    "cancellation_rate": round(
                        (cancelled_count / max(1, total_appointments)) * 100, 1
                    ),
                    "no_show_rate": round(
                        (no_show_count / max(1, total_appointments)) * 100, 1
                    ),
                    "scheduled_rate": round(
                        (scheduled_count / max(1, total_appointments)) * 100, 1
                    ),
                },
            }
            ...
            for provider_id in providers_list:
                if provider_id:  # Skip None values
                    ...
                    provider = (
                        provider_appointments.first().provider
                        if provider_appointments.exists()
                        else None
                    )

                    if provider:
                        total_duration = (
                            provider_appointments.aggregate(
                                total=Sum("duration_minutes")
                            )["total"]
                            or 0
                        )

                        completed_appointments = provider_appointments.filter(
                            status="completed"
                        ).count()
            ...
            frequency_groups = {
                ...
                "2-3 visits": patient_visits.filter(
                    visit_count__gte=2, visit_count__lte=3
                ).count(),
                "4-5 visits": patient_visits.filter(
                    visit_count__gte=4, visit_count__lte=5
                ).count(),
            ...
            patient_first_appointments = appointments.values("patient").annotate(
                first_appointment=Min("appointment_datetime")
            )
            ...
            for patient_data in patient_first_appointments:
                ...
                # Use start_dt and end_dt which are the parsed datetime objects
                start_date_parsed = start_dt.date() if start_dt else None
                end_date_parsed = end_dt.date() if end_dt else None

                if (
                    start_date_parsed
                    and end_date_parsed
                    and start_date_parsed <= first_date <= end_date_parsed
                ):
                    ...
                    # Patient is returning - filter by the datetime objects
                    period_appointments_filter = patient_appointments
                    if start_dt:
                        period_appointments_filter = period_appointments_filter.filter(
                            appointment_datetime__gte=start_dt
                        )
                    if end_dt:
                        period_appointments_filter = period_appointments_filter.filter(
                            appointment_datetime__lte=end_dt
                        )

                    period_appointments_count = period_appointments_filter.count()

                    if period_appointments_count > 0:
                        returning_patients.append(
                            {
                                "patient_id": patient_data["patient"],
                                "appointments_in_period": period_appointments_count,
                            }
                        )
            ...
            return {
                ...
                "ratio": {
                    "new_percentage": round(
                        (
                            len(new_patients)
                            / max(1, len(new_patients) + len(returning_patients))
                        )
                        * 100,
                        1,
                    ),
            ...
            return {
                ...
                "appointment_frequency": {
                    "single_visit": appointments.values("patient")
                    .annotate(count=Count("id"))
                    .filter(count=1)
                    .count(),
                    "multiple_visits": appointments.values("patient")
                    .annotate(count=Count("id"))
                    .filter(count__gt=1)
                    .count(),
                },
            ...
            booked_duration = (
                appointments.aggregate(total=Sum("duration_minutes"))["total"] or 0
            )

            # Get blocked time slots (from availability model)
            blocked_duration = 0
            try:
                blocked_slots = availabilities.filter(is_blocked=True)
            ...
            return {
                "booked_time": {
                    ...
                    "percentage": round(
                        (booked_duration / max(1, total_time)) * 100, 1
                    ),
                },
                "blocked_time": {
                    ...
                    "percentage": round(
                        (blocked_duration / max(1, total_time)) * 100, 1
                    ),
                },
                "total_time_hours": round(total_time / 60, 1),
                "efficiency_ratio": (
                    round(booked_duration / max(1, blocked_duration), 2)
...
class ExportReportView(APIView):
    ...
    def _export_csv(self, report_type, data):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{report_type.replace(" ", "_").lower()}.csv"'
        )
        ...
        if report_type in ["Upcoming Appointments Report", "Past Appointments Report"]:
            ...
            writer.writerow(
                ["Patient Name", "Email", "Registration Date", "Appointment Count"]
            )
            for item in data:
                writer.writerow(
                    [
            ...
            writer.writerow(
                ["Patient", "Provider", "Title", "Recurrence", "Start Date", "End Date"]
            )
            for item in data:
                writer.writerow(
                    [
            ...
            writer.writerow(
                ["Completed", totals["completed"], f"{rates['completion_rate']}%"]
            )
            writer.writerow(
                ["Scheduled", totals["scheduled"], f"{rates['scheduled_rate']}%"]
            )
            writer.writerow(
                ["Cancelled", totals["cancelled"], f"{rates['cancellation_rate']}%"]
            )
            ...
            writer.writerow(
                [
                    "New Patients",
                    data["new_patients"]["count"],
                    f"{data['ratio']['new_percentage']}%",
                ]
            )
            writer.writerow(
                [
                    "Returning Patients",
                    data["returning_patients"]["count"],
                    f"{data['ratio']['returning_percentage']}%",
                ]
            )
            writer.writerow([])  # Empty row
            ...
            writer.writerow(["Metric", "Value"])
            writer.writerow(
                ["Average Lead Time (days)", data["average_lead_time_days"]]
            )
            ...
            writer.writerow(
                ["Single Visit Patients", data["appointment_frequency"]["single_visit"]]
            )
            writer.writerow(
                [
                    "Multiple Visit Patients",
                    data["appointment_frequency"]["multiple_visits"],
                ]
            )
            ...
            writer.writerow(
                [
                    "Booked Time",
                    data["booked_time"]["hours"],
                    data["booked_time"]["minutes"],
                    f"{data['booked_time']['percentage']}%",
                ]
            )
            writer.writerow(
                [
                    "Blocked Time",
                    data["blocked_time"]["hours"],
                    data["blocked_time"]["minutes"],
                    f"{data['blocked_time']['percentage']}%",
                ]
            )
            writer.writerow([])  # Empty row
    ...
    def _export_pdf(self, report_type, data):
        ...
        if report_type in ["Upcoming Appointments Report", "Past Appointments Report"]:
            table_data = [
                ["Patient", "Provider", "Title", "Date & Time", "Duration", "Status"]
            ]
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{report_type.replace(" ", "_").lower()}.pdf"'
        )
