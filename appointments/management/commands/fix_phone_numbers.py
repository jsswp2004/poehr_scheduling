    ...
    def handle(self, *args, **options):
        ...
        for correction in phone_corrections:
            ...
            patients_by_phone = User.objects.filter(
                phone_number=correction["old_phone"], role="patient"
            )
            ...
            # Show what we found
            self.stdout.write(
                f"   Found by phone: {patients_by_phone.count()} patients"
            )
            ...
            for patient in all_matches:
                self.stdout.write(
                    f"   Patient ID {patient.id}: {patient.first_name} {patient.last_name}"
                )
                self.stdout.write(f"     Current phone: '{patient.phone_number}'")
                self.stdout.write(
                    f"     Organization: {patient.organization.name if patient.organization else 'None'}"
                )
