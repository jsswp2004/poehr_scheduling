...
function CalendarView({ onUpdate }) {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const appointmentsPromise = axios
      ...
      const availabilityPromise = axios
        .get("http://127.0.0.1:8000/api/availability/", {
          headers: { Authorization: `Bearer ${token}` },
        })
        .catch((error) => {
          console.error(
      ...
      const availEvents = availabilityRes.data.map((a) => ({
        id: `avail-${a.id}`,
        title: a.is_blocked
          ? `❌ ${a.block_type || "Blocked"} | Dr. ${
              a.doctor_name || "Unknown"
            }`
      ...
      const blockedAvailEvents = availEvents.filter(
  ...
  const getAvailableProvidersForDate = (date) => {
    ...
    return doctors.filter((doctor) => {
      ...
      const hasAvailability = availabilityEvents.some((event) => {
        ...
        return (
          String(eventDoctor) === String(doctor.id) &&
          eventDate === dateStr &&
          !event.is_blocked
        ); // Only count non-blocked availability
      });
  ...
  const handleDateClick = async (date) => {
    ...
    console.log(
      "📝 Sample availabilityEvents:",
      availabilityEvents.slice(0, 3)
    );
    ...
    console.log(
      "📊 All availability events for this date:",
      allAvailabilityForDate
    );

    // Get providers with availability data for this date
    const allProvidersWithAnyAvailability = doctors
      .filter((doctor) => {
        const anySlots = availabilityEvents.filter((event) => {
          ...
          return (
            String(eventDoctor) === String(doctor.id) && eventDate === dateStr
          );
        });
      ...
      .map((doctor) => {
        const allSlots = availabilityEvents
          .filter((event) => {
            if (event.type !== "availability") return false;
    ...
    console.log(
      "🏥 All providers with ANY availability (blocked or not):",
    ...
    if (currentUserRole === "patient") {
      ...
      try {
        ...
        if (assignedProviderId) {
          ...
          const assignedProviderWithAvailability =
            allProvidersWithAnyAvailability.find((provider) => {
              return (
                String(provider.id) === String(assignedProviderId) &&
          ...
          if (assignedProviderWithAvailability) {
            ...
            const availableSlots =
              assignedProviderWithAvailability.timeSlots.filter(
                (slot) => !slot.isBlocked
              );
            providersToShow = [
              {
            ...
            console.log(
              "✅ Showing assigned provider with available slots:",
              providersToShow
            );
          } else {
        ...
        console.error(
          "❌ Failed to fetch user profile for patient filtering:",
          err
        );
        // Fallback to showing all providers with available slots
        const providersWithAvailableSlots =
          allProvidersWithAnyAvailability.filter((provider) => {
      ...
      providersToShow =
        allProvidersWithAnyAvailability.length > 0
          ? allProvidersWithAnyAvailability
          : doctors.map((doctor) => ({
              ...doctor,
              timeSlots: [
    ...
    console.log(
      "📋 Final providers to show in modal (after role filtering):",
      providersToShow
    );
  ...
  const checkAvailabilityConflict = useCallback(
    (startDate, durationMinutes, doctorId) => {
      ...
      if (isEditing && editingId) {
        ...
        if (originalEvent) {
          ...
          const timeUnchanged =
            Math.abs(originalStart.getTime() - start.getTime()) < 60000 && // within 1 minute
    ...
    [
      modalFormData.appointment_datetime,
  ...
  const handleModalSave = async () => {
    ...
    try {
      if (isEditing && editingId) {
        ...
        await axios.put(
          `http://127.0.0.1:8000/api/appointments/${cleanId}/`,
          payload,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
  ...
  const eventStyleGetter = (event) => {
    ...
    if (event.type === "availability") {
      backgroundColor = event.is_blocked ? "#f8d7da" : "#d1e7dd";
    }
    return {
      style: {
        backgroundColor,
  ...
  useEffect(() => {
    const loadProviderBlocks = async () => {
      ...
      try {
        const res = await axios.get(
          `http://127.0.0.1:8000/api/availability/?doctor=${selectedDoctor.value}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  return (
    <Box
      sx={{ mt: 1, boxShadow: 2, borderRadius: 2, bgcolor: "background.paper" }}
    >
        ...
        <Box sx={{ height: 600, maxWidth: "100%" }}>
          <Calendar
            ...
            events={events
              .filter((event) => {
                if (
                  userRole === "patient" &&
                  event.id.toString().startsWith("avail")
                ) {
                  return false;
                }
              ...
              .filter((event) => {
                if (
                  ...
                  return (
                    String(event.doctor_id) === String(selectedDoctor.value)
                  );
                }
                return true;
              })}
          ...
          >
            ...
            <DialogContent dividers>
              ...
              <Stack spacing={2} mt={2}>
                ...
                <FormControl fullWidth size="small">
                  <InputLabel id="clinic-event-label">
                    Clinic Visit Type
                  </InputLabel>
                  <MUISelect
                    ...
                    onChange={(e) => {
                      const selected = clinicEvents.find(
                    ...
                    onChange={(e) => {
                      ...
                      setSelectedDoctor(
                        selected
                          ? {
                              value: selected.id,
                              label: `Dr. ${selected.first_name} ${selected.last_name}`,
                            }
                          : null
                      );
                    }}
            ...
            <DialogActions>
              ...
              <Tooltip
                title={
                  isEditing ? "Update appointment" : "Save new appointment"
                }
              >
                <span>
                  {" "}
