...
const MessagesModal = ({
    ...
    // Get messages for selected user - simplified
    const messages = useMemo(() => {
        if (!selectedUser) return [];
        return getUserMessages(selectedUser.id || selectedUser.user_id);
    ...
    // Mark user as read when selected - simplified
    useEffect(() => {
        if (selectedUser) {
