...
function AutoEmailSetUpPage() {
  ...
  const fetchMonthlyTotal = async () => {
    try {
      ...
      const res = await axios.get(
        `http://127.0.0.1:8000/api/communicator/logs/?message_type=email&created_at__gte=${startDate}&created_at__lte=${endDate}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
        ...
        >
          ...
          <Stack spacing={2} sx={{ maxWidth: 300 }}>
            ...
            {runNowStatus && (
              <Alert
                severity={
                  runNowStatus === "Emails are being sent!" ? "success" : "info"
                }
              >
                {runNowStatus}
              </Alert>
            ...
            >
              <Typography variant="body2" color="text.secondary">
                Total emails sent for{" "}
                {new Date().toLocaleDateString("en-US", {
                  month: "long",
                  year: "numeric",
                })}
                : <strong>{monthlyEmailTotal}</strong>
              </Typography>
            </Box>
          </Stack>
        </Box>
