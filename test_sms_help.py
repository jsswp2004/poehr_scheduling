...
def test_help_functionality():
    ...
    test_scenarios = [
        {
            ...
            "setup": lambda u: setattr(u, 'sms_consent', True) or setattr(u, 'sms_opt_out', False) or u.save(),
            "body": "HELP"
        },
        {
            "name": "User who has opted out", 
            "setup": lambda u: setattr(u, 'sms_opt_out', True) or setattr(u, 'sms_opt_out_method', 'STOP') or u.save(),
            "body": "HELP"
        },
        {
            "name": "User with no consent",
            "setup": lambda u: setattr(u, 'sms_consent', False) or setattr(u, 'sms_opt_out', False) or u.save(),
            "body": "HELP"
        },
        {
            "name": "INFO keyword",
    ...
    for scenario in test_scenarios:
        ...
        if scenario['setup']:
            scenario['setup'](user)
            print(f"Setup complete: consent={user.sms_consent}, opt_out={user.sms_opt_out}")
        
        # Create webhook request
        request = factory.post('/api/communicator/sms-webhook/', {
